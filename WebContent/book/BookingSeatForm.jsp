...
String MyCtrlCommand = contextPath + CommandName + "?command=";
    ...
    String _totalSeat = request.getParameter("totalSeat");
    String ts = request.getParameter("ts");
    int length = Integer.parseInt(request.getParameter("leng"));
    StringTokenizer st = new StringTokenizer(ts);
    
    String[] reservSeat = new String[length];
    int count = 0;
    while(st.hasMoreElements()){
     reservSeat[count] = st.nextToken(",");
     count++;
    }
...
function send() {
	...
	function get_chked_values(){
		...
		opener.document.myform.hiddenseat.value = chked_val;
		opener.document.myform.price.value = cnt*<%=price%>;
		opener.document.myform.amount.value = cnt;
...
<form action="/Cinema//book/BookingAction.jsp"  name="myform" id="myform" class="myform" method="post">
...
<%
	...
	for(int i=1; i<=row; i++) {
		...
		for(int j=1; j<=col; j++) {
			...
			s=(char)(i+64)+":"+j;
int ab = 0;
			 for(int k = 0; k<reservSeat.length; k++){
			 
			if(ab==0){
	 		if(s.equals(reservSeat[k])){
			out.print("<input type='checkbox' class='ch' disabled='disabled' name='ch' value='"+
